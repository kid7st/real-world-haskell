module Ch3
  ...
  , heightOfBTree
  , Point
  , Direction(DLeft, DStraight, DRight)
  , direction
  , directions
  , sortedPoints
  , graham
...
heightOfBTree (Node _ left right) =
  1 + max (heightOfBTree left) (heightOfBTree right)
...
sortedPoints points = minP : sortBy cmp (filter (/= minP) points)
  where
    minP = getMinPoint points
      where
        getMinPoint (p:ps) = foldl minPoint p ps
          where
            minPoint (x1, y1) (x2, y2)
              | y1 < y2 = (x1, y1)
              | y1 > y2 = (x2, y2)
              | x1 < x2 = (x1, y1)
              | otherwise = (x2, y2)
